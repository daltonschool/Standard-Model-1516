 ...
 */
public class NewDriveTwo extends OpMode {

    final double antiJerkVal = Math.sin(Math.PI/4) * 2; //if this is still too jerky, decrease the the value
...
//            }
        ...
        if(Math.abs(gamepad1.right_trigger) > 0.2) {
            hook1.setPosition(0.5 + gamepad1.right_trigger/2);
            hook2.setPosition(0.5 - gamepad1.right_trigger/2);
        } else if(Math.abs(gamepad1.left_trigger) > 0.2) {
            hook1.setPosition(0.5 - gamepad1.left_trigger/2);
            hook2.setPosition(0.5 + gamepad1.left_trigger/2);
        } else {
        }
